!function (Z) {
    ...
    Z.fn.bxSlider = function (e) {
        ...
        var g = {}, p = this, n = Z(window).width(), s = Z(window).height();
        if (!Z(p).data("bxSlider")) {
            var o = function () {
                Z(p).data("bxSlider") || (g.settings = Z.extend({}, U, e), g.settings.slideWidth = parseInt(g.settings.slideWidth), g.children = p.children(g.settings.slideSelector), g.children.length < g.settings.minSlides && (g.settings.minSlides = g.children.length), g.children.length < g.settings.maxSlides && (g.settings.maxSlides = g.children.length), g.settings.randomStart && (g.settings.startSlide = Math.floor(Math.random() * g.children.length)), g.active = {index: g.settings.startSlide}, g.carousel = 1 < g.settings.minSlides || 1 < g.settings.maxSlides, g.carousel && (g.settings.preloadImages = "all"), g.minThreshold = g.settings.minSlides * g.settings.slideWidth + (g.settings.minSlides - 1) * g.settings.slideMargin, g.maxThreshold = g.settings.maxSlides * g.settings.slideWidth + (g.settings.maxSlides - 1) * g.settings.slideMargin, g.working = !1, g.controls = {}, g.interval = null, g.animProp = "vertical" === g.settings.mode ? "top" : "left", g.usingCSS = g.settings.useCSS && "fade" !== g.settings.mode && function () {
                    for (var t = document.createElement("div"), e = ["WebkitPerspective", "MozPerspective", "OPerspective", "msPerspective"], i = 0; i < e.length; i++) if (void 0 !== t.style[e[i]]) return g.cssPrefix = e[i].replace("Perspective", "").toLowerCase(), g.animProp = "-" + g.cssPrefix + "-transform", !0;
                    return !1
                ...
                p.wrap('<div class="' + g.settings.wrapperClass + '"><div class="bx-viewport"></div></div>'), g.viewport = p.parent(), g.settings.ariaLive && !g.settings.ticker && g.viewport.attr("aria-live", "polite"), g.loader = Z('<div class="bx-loading" />'), g.viewport.prepend(g.loader), p.css({
                    ...
                    overflow: "hidden",
                    position: "relative"
                }), g.viewport.parent().css({maxWidth: l()}), g.children.css({
                ...
                var e = 0, t = Z();
                if ("vertical" === g.settings.mode || g.settings.adaptiveHeight) if (g.carousel) {
                ...
                } else t = g.children.eq(g.active.index); else t = g.children;
                ...
                var t = g.settings.slideWidth, e = g.viewport.width();
                if (0 === g.settings.slideWidth || g.settings.slideWidth > e && !g.carousel || "vertical" === g.settings.mode) t = e; else if (1 < g.settings.maxSlides && "horizontal" === g.settings.mode) {
                ...
                var e, i, n = t || g.settings.speed, s = {left: 0, top: 0}, o = {left: 0, top: 0};
                "next" === g.settings.autoDirection ? s = p.find(".bx-clone").first().position() : o = g.children.first().position(), e = "horizontal" === g.settings.mode ? -s.left : -s.top, i = "horizontal" === g.settings.mode ? -o.left : -o.top, x(e, "ticker", n, {resetValue: i})
            }, O = function (t) {
                ...
                if ("touchstart" === t.type || 0 === t.button) if (t.preventDefault(), g.controls.el.addClass("disabled"), g.working) g.controls.el.removeClass("disabled"); else {
                ...
                var e = t.originalEvent, i = void 0 !== e.changedTouches ? e.changedTouches : [e],
                    n = Math.abs(i[0].pageX - g.touch.start.x), s = Math.abs(i[0].pageY - g.touch.start.y), o = 0,
                    r = 0;
                ...
                if (g.initialized) if (g.working) window.setTimeout(V, 10); else {
                    var e = Z(window).width(), i = Z(window).height();
            ...
            return p.goToSlide = function (t, e) {
                var i, n, s, o, r, a = !0, l = 0, d = {left: 0, top: 0}, c = null;
                if (g.oldIndex = g.active.index, g.active.index = (r = t) < 0 ? g.settings.infiniteLoop ? h() - 1 : g.active.index : r >= h() ? g.settings.infiniteLoop ? 0 : g.active.index : r, !g.working && g.active.index !== g.oldIndex) {
                    ...
                    "next" === e ? g.settings.onSlideNext.call(p, g.children.eq(g.active.index), g.oldIndex, g.active.index) || (a = !1) : "prev" === e && (g.settings.onSlidePrev.call(p, g.children.eq(g.active.index), g.oldIndex, g.active.index) || (a = !1)), g.active.last = g.active.index >= h() - 1, (g.settings.pager || g.settings.pagerCustom) && y(g.active.index), g.settings.controls && A(), "fade" === g.settings.mode ? (g.settings.adaptiveHeight && g.viewport.height() !== u() && g.viewport.animate({height: u()}, g.settings.adaptiveHeightSpeed), g.children.filter(":visible").fadeOut(g.settings.speed).css({zIndex: 0}), g.children.eq(g.active.index).css("zIndex", g.settings.slideZIndex + 1).fadeIn(g.settings.speed, function () {
                        Z(this).css("zIndex", g.settings.slideZIndex), I()
                    })) : (g.settings.adaptiveHeight && g.viewport.height() !== u() && g.viewport.animate({height: u()}, g.settings.adaptiveHeightSpeed), !g.settings.infiniteLoop && g.carousel && g.active.last ? "horizontal" === g.settings.mode ? (d = (c = g.children.eq(g.children.length - 1)).position(), l = g.viewport.width() - c.outerWidth()) : (i = g.children.length - g.settings.minSlides, d = g.children.eq(i).position()) : g.carousel && g.active.last && "prev" === e ? (n = 1 === g.settings.moveSlides ? g.settings.maxSlides - v() : (h() - 1) * v() - (g.children.length - g.settings.maxSlides), d = (c = p.children(".bx-clone").eq(n)).position()) : "next" === e && 0 === g.active.index ? (d = p.find("> .bx-clone").eq(g.settings.maxSlides).position(), g.active.last = !1) : 0 <= t && (o = t * parseInt(v()), d = g.children.eq(o).position()), void 0 !== d && (s = "horizontal" === g.settings.mode ? -(d.left - l) : -d.top, x(s, "slide", g.settings.speed)), g.working = !1), g.settings.ariaHidden && R(g.active.index * v())
                }
...
function prodCalc(n) {
    var t, e, o = $(n).find(".pc-amount"), a = $(n).find(".cl-i.active").text(), i = $(n).find(".p-cost"),
        s = (t = $(n).attr("data-start"), e = u("base", ""), +t + +e), r = u("option", ":checked"), d = s + r,
        c = $(n).find(".old-o-cost"), l = $(n).attr("data-sale");

    function u(t, e) {
        t = "data-" + t;
        var o = $(n).find("[" + t + "]" + e);
        return +(o = o.is(":empty") || o.is("div") ? o.attr(t).split(a + "=")[1].split(" ")[0] : 0)
    }
    ...
    if (f !== p) {
        var h = 30, m = null;
        p < f && (m = p + 10 - 1, $(i).addClass("c-attention-m"), setTimeout(function () {
            $(i).removeClass("c-attention-m")
        }, 100)), f < p && (m = p - 10 + 1, $(i).addClass("c-attention-p"), setTimeout(function () {
            $(i).removeClass("c-attention-p")
        }, 100)), function t(e) {
            setTimeout(function () {
                p < f && ($(o).text(number_format(m, 0, ".", " ")), m = --m), f < p && ($(o).text(number_format(m, 0, ".", " ")), m = ++m), --e && t(e), h = +h + 15
            }, +h)
        }(10)
    }
    setTimeout(function () {
...
function number_format(t, e, o, n) {
    t = (t + "").replace(/[^0-9+\-Ee.]/g, "");
    var a, i, s, r = isFinite(+t) ? +t : 0, d = isFinite(+e) ? Math.abs(e) : 0, c = void 0 === n ? "," : n,
        l = void 0 === o ? "." : o, u = "";
    return 3 < (u = (d ? (a = r, i = d, s = Math.pow(10, i), "" + (Math.round(a * s) / s).toFixed(i)) : "" + Math.round(r)).split("."))[0].length && (u[0] = u[0].replace(/\B(?=(?:\d{3})+(?!\d))/g, c)), (u[1] || "").length < d && (u[1] = u[1] || "", u[1] += new Array(d - u[1].length + 1).join("0")), u.join(l)
}

function substituter(t, e) {
    var o = $(t).next(".p-cost").find(".pc-amount").text(), n = +(o = o.replace(/\s+/g, "")) + +(e = o / 100 * e);
    $(t).find(".old-pc-amount").text(number_format(n, 0, ".", " "))
}

document.body.classList.add("js"), $(function () {
    var t = $(".top-wrapper");
...
}), $(".products").on("click", function (t) {
    var e = t.target, o = $(e).parents(".product");
    if ($(e).hasClass("cl-i")) return $(e).text(), $(e).parent().children(".cl-i").removeClass("active"), $(e).addClass("active"), prodCalc(o), void n();
    if ($(e).is("input")) return prodCalc(o), void n();

    function n() {
        var t = $(o).find(".add");
        $(t).hasClass("added") && $(t).removeClass("added").text("В корзину")
    }

    $(e).hasClass("info") && $(e).tooltipster("open")
}), function () {
    var t = $(".form-wrapper"), n = t.find(".form-message"), a = t.find(".form-body");

    function i() {
        t.fadeOut(200), $("body").css("overflow", "auto")
    }

    t.on("click", function (t) {
        var e, o = t.target;
        if ($(o).hasClass("c-close")) i(); else {
            if (o != this) return $(o).hasClass("form-btn") ? (event.preventDefault(), e = "phone=" + $("#form-phone").val(), void $.ajax({
                type: "POST",
                url: "assets/php/form.php",
                data: e,
                success: function (t) {
                    var e = "";
                    "OK" == t ? (e = "<h3>Запрос оставлен</h2><p>Мы вам позвоним в ближайшее рабочее время. Работаем с 9⁰⁰ до 21⁰⁰.</p>", a.hide(), n.html(e), setTimeout(function () {
                        i()
                    }, 5e3)) : (e = t, n.html(e))
                }
            })) : void 0;
        ...
        a.show(), n.empty(), $(".error").removeClass("error"), $(".input-error").remove(), t.find("input[type=text], textarea").val(""), t.fadeIn(200), $("body").css("overflow", "hidden")
    })
}(), $("body").on("click", '[href*="#"]', function (t) {
    $("html,body").stop().animate({scrollTop: $(this.hash).offset().top - 50}, {
        duration: 1e3,
        easing: "swing"
    }), t.preventDefault()
}), function () {
    $(".top-menu-btn");
    var o = $(".top-links"), n = $("body");

    function a() {
        $(o).fadeOut(200), setTimeout(function () {
            o.removeClass("showed")
        }, 200), $(n).removeClass("scroll-off")
    }

    n.on("touchstart  mousedown", function (t) {
        var e = t.target;
        $(e).hasClass("top-menu-btn") && ($(o).fadeIn(200).addClass("showed"), $(n).addClass("scroll-off")), $(e).hasClass("menu-close-btn") && a(), $(e).is(".top-links a") && $(window).width() < 800 && a(), $(e).is(":not(.top-links):not(.top-menu-btn)") && $(window).width() < 800 && a()
    }), $(window).resize(function () {
        800 < $(window).width() ? o.show() : o.hide()
    })
}(), function () {
    var t = $(".one-click");
    $(window).scroll(function () {
        700 < $(this).scrollTop() ? t.fadeIn(200) : t.fadeOut(200)
    })
}(), $(".product[data-sale]").each(function (t, e) {
    var o = $(e).attr("data-sale");
    substituter($(e).find(".old-o-cost"), o)
});
